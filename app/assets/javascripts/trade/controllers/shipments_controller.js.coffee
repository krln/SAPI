Trade.ShipmentsController = Ember.ArrayController.extend Trade.QueryParams,
  ...
  autoCompleteTaxonConcepts: ( ->
    ...
    if !taxonConceptQuery || taxonConceptQuery.length < 3
      return [];
    @store.find('autoCompleteTaxonConcept',
      taxonomy: 'CITES'
  ...
  autoCompleteTaxonConceptsByRank: ( ->
    ...
    @get('autoCompleteTaxonConcepts.meta.rank_headers').map (rh) ->
      ...
      taxon_concepts: rh.taxon_concept_ids.map (tc_id) ->
        @store.find('autoCompleteTaxonConcept', tc_id)
  ...
  actions:
    ...
    newShipment: () ->
      @set('currentShipment', @store.createRecord('shipment'))
      $('.modal').modal('show')
    ...
    deleteShipment: (shipment) ->
      ...
      shipment.save().then(
        =>
          ...
          @send('search')
        (error) ->
          #Not succesfull, rollback the delete action
          shipment.rollback()
